...
if($_GET['tipo'] == 1){
    ...
    $consulta = "SELECT id_gastos, fecha_pago, descripcion, precio FROM gastos";

}else{
    $consulta = "SELECT id_gastos, fecha_pago, descripcion, precio FROM gastos WHERE (fecha_pago BETWEEN '".$_GET['fI']."' AND '".$_GET['fF']."')";
...
// Set properties
    ...
    $objPHPExcel->getProperties()->setTitle("Reporte Historial Gastos Caporce");
    $objPHPExcel->getProperties()->setSubject("Reporte Historial Gastos");
    $objPHPExcel->getProperties()->setDescription("Reporte Historial Gastos");
...
//Nos va servir para obtener los labels de cada campo
    ...
    $tituloReporte = "Historial Gastos CAPORCE";
    $titulosColumnas = array('Folio de Gasto','Fecha de Gasto','Descripcion','Monto Total');
    ...
    while ($fila = mysqli_fetch_array($resultado)){
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue("A" . $counter, $fila['id_gastos']);
        $objPHPExcel->getActiveSheet()->SetCellValue("B" . $counter, $fila['fecha_pago']);
...
//forma sencilla de dar estilos
    ...
    $nombreArchivo = "ReporteHistorialGastos". date('d-m-Y');
